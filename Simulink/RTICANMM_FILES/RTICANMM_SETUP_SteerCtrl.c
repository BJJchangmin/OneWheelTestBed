...
#define S_FUNCTION_NAME        RTICANMM_SETUP_SteerCtrl                                                                                          
...
#include "RTICANMM_SETUP_SteerCtrl.h"                                                                                                    
...
static Float64 CANMM_CTL_SETUP_SteerCtrl_baudrate      = 500.0;                                                                                 
static Float64 CANMM_CTL_SETUP_SteerCtrl_baudrate_in   = 500.0; /* Temporary for input port value */                                            
static Float64 CANMM_CTL_SETUP_SteerCtrl_baudrate_old  = 500.0;                                                                                 
...
volatile UInt8 CANMM_CTL_SETUP_SteerCtrl_ID;                                                                                              
volatile UInt8 CANMM_CTL_SETUP_SteerCtrl_TX_STATUS                 = 1;                                                                   
volatile UInt8 DECLSPEC CANMM_CTL_SETUP_SteerCtrl_Variation        = 1;                                                                   
volatile UInt8 DECLSPEC CANMM_CTL_SETUP_SteerCtrl_VariationSwitch  = 0;                                                                   
...
#endif                                                                                                           
volatile UInt8   CANMMMonitor_SteerCtrl     = 0;                                                                                     
volatile UInt8   CANMMMonitor_SteerCtrl_old = 0;                                                                                     
volatile dsfloat MonitoringTimeStamp_SteerCtrl = 0.0;                                                                                       
...
static CANMM_CTL_CLIENT_TYPE(CANMM_CTL_SETUP_SteerCtrl) ctlsetup_client;                                                                
#endif /* RTICANMM_DSDAQ_SERVICEID */                                                                            
                                                                                                                 
                                                                                                                   
void                                                                                                               
CANMM_CTL_SETUP_SteerCtrl_baudrate_set(Float64 baudrate) {                                                                                
  ...
  if ((Error = DsCanCh_apply(CanMM_channel)) != DSCAN_NO_ERROR)                                                    
    ...
    CANMM_CTL_SETUP_SteerCtrl_baudrate = baudrate;                                                                                        
  }                                                                                                                
}                                                                                                                  
...
#if defined(RTICANMM_HAVE_VALID_PLATFORM)                                                                          
static SimStruct * RTICANMM_SETUP_SteerCtrl_SimStruct = 0;                                                                               
void backgroundFunction_12143421()                                                                                       
{                                                                                                                  
  SimStruct *S = RTICANMM_SETUP_SteerCtrl_SimStruct;                                                                                     
...
void DECLSPEC CANMM_CTL_SETUP_SteerCtrl_client_init(CANMM_CTL_SETUP_SteerCtrl_CtlClient* client) {                                                               
  ...
  if (!client->initialized) {                                                                                      
    ...
    if ((Error = DsCanMsg_setQueueSize(client->tx_msg, CANMM_CTL_SETUP_SteerCtrl_tx_queue_size)) != DSCAN_NO_ERROR)                       
      CANMM_DEBUG_ERROR(Error);                                                                                    
#if (defined(DSCAN_VER_MAR) && DSCAN_VER_MAR == 1) || defined(RTICANMM_HAVE_DSRT)                                  
 ...
 * If CANMM_CTL_SETUP_SteerCtrl_rx_stop has been issued before, the receive buffer (mcr) is cleared                                       
 ...
 * CANMM_CTL_SETUP_SteerCtrl_rx function. To continue reception of messages CANMM_CTL_SETUP_SteerCtrl_rx_start                                                   
 * has to be called.                                                                                               
 */                                                                                                                
...
Float64 DECLSPEC CANMM_CTL_SETUP_SteerCtrl_baudrate_get() {                                                                               
  return CANMM_CTL_SETUP_SteerCtrl_baudrate;                                                                                              
}                                                                                                                  
...
UInt32 DECLSPEC CANMM_CTL_SETUP_SteerCtrl_tx_queue_level(CANMM_CTL_SETUP_SteerCtrl_CtlClient* client) {                                                          
  ...
  if ((Error = DsCanMsg_getTxQueueCount(client->tx_msg, &tx_count)) != DSCAN_NO_ERROR) {                           
    ...
    tx_count = CANMM_CTL_SETUP_SteerCtrl_tx_queue_size;                                                                                   
  }                                                                                                                
  return tx_count;                                                                                                 
}                                                                                                                  
...
# endif /* RTICANMM_HAVE_DSCAN_BUS */                                                                              
  ...
  if ((Error = DsCanCh_setDescriptor(CanMM_channel, "SteerCtrl")) != DSCAN_NO_ERROR)                                      
    CANMM_DEBUG_ERROR(Error);                                                                                      
                                                                                                                   
  if ((Error = DsCanCh_setBaudRate(CanMM_channel, (int)CANMM_CTL_SETUP_SteerCtrl_baudrate*1000)) != DSCAN_NO_ERROR)                   
      CANMM_DEBUG_ERROR(Error);                                                                                
  ...
  if(S != 0)                                                                                                      
       RTICANMM_SETUP_SteerCtrl_SimStruct = S;                                                                                          
...
# endif                                                                                                            
#if defined(RTICANMM_DSDAQ_SERVICEID)																				  
    CANMM_CTL_CLIENT_INIT(CANMM_CTL_SETUP_SteerCtrl, &ctlsetup_client);                                                                   
    CANMM_CTL_RX_STOP(CANMM_CTL_SETUP_SteerCtrl, &ctlsetup_client);      /* Start disabled */                                             
# endif                                                                                                            
...
/****************************/                                                                                     
  ...
  if(S != 0)                                                                                                       
      RTICANMM_SETUP_SteerCtrl_SimStruct = S;     //just to be safe we do one more assigment                                             
#endif /* RTICANMM_HAVE_VALID_PLATFORM */                                                                          
}                                                                                                                          
#endif /*  MDL_START */                                                                                                    
...
#if defined(RTICANMM_DSDAQ_SERVICEID)                                                                            
if (CANMMMonitor_SteerCtrl > 0 && !(CANMMMonitor_SteerCtrl_old > 0)) { /* monitoring enabled */                                                          
  CANMM_CTL_RX_START(CANMM_CTL_SETUP_SteerCtrl, &ctlsetup_client);                                                                      
}                                                                                                                
if (!(CANMMMonitor_SteerCtrl > 0) && CANMMMonitor_SteerCtrl_old > 0) { /* monitoring disabled */                                                         
  CANMM_CTL_RX_STOP(CANMM_CTL_SETUP_SteerCtrl, &ctlsetup_client);                                                                       
}                                                                                                                
CANMMMonitor_SteerCtrl_old = CANMMMonitor_SteerCtrl;                                                                                                     
                                                                                                                 
if (CANMMMonitor_SteerCtrl > 0) {                                                                                                    
  while (CANMM_CTL_RX(CANMM_CTL_SETUP_SteerCtrl, &ctlsetup_client, &canmm_msg)) {                                                       
    UInt32 i = 0;                                                                                                
    int j = 0, k = 0;                                                                                            
...
#endif                                                                                                           
    MonitoringTimeStamp_SteerCtrl = canmm_msg.TimeStamp;                                                                                    
  ...
  if (CANMM_CTL_SETUP_SteerCtrl_VariationSwitch == 0) {                                                                                   
    CANMM_CTL_SETUP_SteerCtrl_ID = INDEF_VARIATION;                                                                                       
  }                                                                                                                
  else {                                                                                                           
    CANMM_CTL_SETUP_SteerCtrl_ID = CANMM_CTL_SETUP_SteerCtrl_Variation;                                                                                          
  }                                                                                                                
...
/****************************/                                                                                     
  ...
  CANMMMonitor_SteerCtrl_old = 0;                                                                                                      
  if ((Error = DsCanCh_stop(CanMM_channel)) != DSCAN_NO_ERROR)                                                     
    CANMM_DEBUG_ERROR(Error);                                                                                      
    RTICANMM_SETUP_SteerCtrl_SimStruct = 0;                                                                                              
#endif   /* RTICANMM_HAVE_VALID_PLATFORM */                                                                        
}                                                                                                                          
